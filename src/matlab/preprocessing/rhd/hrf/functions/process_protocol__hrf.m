function process_protocol__hrf(protocollist,rhdlist,outputdir,rootdir,nVolume)
% PROCESS_PROTOCOL__HRF Historical HRF RHD/protocol synchronization.
%
% Adapted only at the interface/name level from the supplied historical
% function named process_protocol__reappraisal. Its contents are clearly
% HRF-specific (8200-volume ultrafast acquisition, ~265-ms triggers and
% 0.5/1.0/2.4-s odor durations), so the scientific timing logic is retained.
% Absolute project paths are supplied by the calling master.
...
for ii=1:size(pairs,2)
    ...
    % fv_dur (here: 2.4 s)
    Crit1 = ( fv_dur < 2.41 & fv_dur > 2.39); % Duration: 2.4s
    Crit2 = ( fv_dur < 0.51 & fv_dur > 0.49); % Duration: 0.5s
    Crit3 = ( fv_dur < 1.01 & fv_dur > 0.99); % Duration: 1s
    ToDelete = find(~(Crit1+Crit2+Crit3));
    ...
    for zx=1:(size(trigTime_off,1)-1);
        ...
        if trigDurTotal(zx) < 0.26 % TR is 265 ms, 0.265
    ...
    % as a starting point
    if ~isempty(find(trigDurTotal>0.28,1))
        trigDurExceeded_all=find(trigDurTotal>0.28); % 0.28 is arbitrary, but was chosen because it is unlikely that the distance between two regular EPI triggers (as opposed to adjustment triggers) will be more than 3.4 (this could only happen if there are at least 2 adjacent gaps in the data each taking away a trigger)
    ...
    if exist('trigStartIx','var')
        trigStartIx_plot = trigStartIx;
    else
        trigStartIx_plot = 0;
    end
    plot([zeros(trigStartIx_plot,1); trigTime_on], 'b' )
    ...
    if length(trigTime_on) > nVolume
        ...
        trigTime=trigTime_off %trigTime_on; %% CAVE: Actually, the EPI is starting AFTER the trigger, therefore, trigTime_off would be more correct than trigTime_on in here
        trigDur=trigTime_off-trigTime_on;
    
        for zx=1:(size(trigTime_off,1)-1);
            trigDurInter(zx)=trigTime_on(zx+1)-trigTime_off(zx);
            trigDurTotal(zx)=trigDur(zx)+trigDurInter(zx);
            if trigDurTotal(zx) < 0.26
                display(['trigDur of # ' num2str(zx) 'to small in' protocolfile '.mat']);
            end;
        end;
    ...
    ToCorrect=find(trigDurTotal<0.26);
    ...
    ToDelete= find( trigInter >0.27 | trigInter < 0.26);
    
    % This is added since sometime triggers at the end of the scan are
    % found:
    ToDelete=ToDelete(find(ToDelete<8200));
    ...
    scan_start_del25=trigTime(26); %tp when the aquisition of the 6th EPI starts LW
    ...
    hold on; h2=line([trigTime(26)*freq trigTime(26)*freq],[0 2.2]); h2.Color=[0 1 0];h2.LineStyle='-.';h2.LineWidth=1;
    ...
    hold on; h4=line([fv_on(1)*freq fv_on(1)*freq],[0 2.2]); h4.Color=[1 1 0];h1.LineStyle='-';h4.LineWidth=1;
    title(cellstr(protocolfile),'Interpreter', 'none');
    l=legend('EPI-trigger','Odor-On/Off','EPI-START','EPI-del25START','Paradigm-END','Paradigm-START','Location','southeast');
    l.FontSize=7;
    ...
    MyText{2}=string(['Scan Start (del25): ' num2str(scan_start_del25) 'ms']);
    ...
    fv_on_del25=fv_on-scan_start_del25;
    if fv_on(1) < scan_start_del25
        error(['first odor before end of dummies in ' protocolfile '.mat'])
    end;
    fv_off_del25=fv_off-scan_start_del25;
    trigTime_del25=trigTime-scan_start_del25;
    ...
    for i=1:length(fv_on)-1
        ...
        events(i).fv_on_del25=fv_on_del25(i);
        events(i).fv_off_del25=fv_off_del25(i);
        events(i).fv_dur_del25=fv_dur(i);
    ...
    info.scan_start_del25=scan_start_del25;
