function process_protocol_reappraisal(protocollist,rhdlist,outputdir,rootdir,nVolume) % [events, info, licks] =
...
for ii=1:size(pairs,2)
    ...
    ADCs=get_all_files_recursive(rootdir,'*adc.mat'); load(ADCs{1});
    DIGs=get_all_files_recursive(rootdir,'*digital.mat'); load(DIGs{1});
    ...
    %%%
    outps=fullfile(outputdir,['trigger_check_figs_' datestr(now,'yyyymmdd') '.ps']);
    outtxt=fullfile(outputdir,['trigger_check_' datestr(now,'yyyymmdd') '.txt']);
