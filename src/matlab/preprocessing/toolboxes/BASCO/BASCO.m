...
if AnaDef.ROIAnalysis==true % retrieve ROIs
    ...
    if size(thenames,1)~=ROINum % changed by JR from length(thenames)
...
for isubj=1:NumSubj % loop over subjects  %%%%%%%%%%%%%%%%%%%%%
    ...
    for irun=1:NumRuns % loop over runs
        ...
        counter  = 0;
        for icond=1:AnaDef.NumCond(irun) % loop over conditions %% CHANGED by JR from icond=1:AnaDef.NumCond
            ...
            for ionsets=1:1:NumOnsets % loop over individual trials of given condition
                ...
                counter=counter+1;
                if NumRuns==1 %% if loop added by JR
                ...
                elseif NumRuns>1 %% if loop added by JR
                    matlabbatch{2}.spm.stats.fmri_spec.sess(irun).cond(counter).name     = sprintf('%s%d',AnaDef.Cond{irun}{icond},ionsets);
                    matlabbatch{2}.spm.stats.fmri_spec.sess(irun).cond(counter).onset    = trialonset;
                    matlabbatch{2}.spm.stats.fmri_spec.sess(irun).cond(counter).duration = AnaDef.Subj{isubj}.Duration{irun}(icond);
                    matlabbatch{2}.spm.stats.fmri_spec.sess(irun).cond(counter).tmod     = 0;
                    matlabbatch{2}.spm.stats.fmri_spec.sess(irun).cond(counter).pmod     = struct('name', {}, 'param', {}, 'poly', {});
                    % store information on regressors and the corresponding condition
                    regcounter=regcounter+1;
        ...
        % motion regressors
        %
%         listmot   = spm_select('FPList',fullfile(data_path,AnaDef.Subj{isubj}.RunDirs{irun}), ['^rp_*.*\.txt']);% CHANGED by JR
        if NumRuns==1 %% if loop added by JR
            listmot   = spm_select('FPList',fullfile(data_path,AnaDef.Subj{isubj}.RunDirs{irun}),AnaDef.Subj{cSubj}.Covariates);% CHANGED by JR
        elseif NumRuns>1 %% if loop added by JR
            listmot   = spm_select('FPList',fullfile(data_path,AnaDef.Subj{isubj}.RunDirs{irun}),AnaDef.Subj{cSubj}.Covariates{irun});% CHANGED by JR
        end
        ...
        brainmask = 'mask_template_6_polished_roi.mat'
        roifiles    = [ '' ];
        ...
        if AnaDef.GlobalMeanReg==true && AnaDef.MotionReg==true && AnaDef.CSFReg==true
            matlabbatch{2}.spm.stats.fmri_spec.sess(irun).multi_reg = cellstr(filereg);
            if AnaDef.DerivReg
                regcounter = regcounter+1+6*2+1*2; % increase regressor-counter
            else
                regcounter = regcounter+1+6*1+1*1; % increase regressor-counter
            end
        end
        if AnaDef.GlobalMeanReg==false && AnaDef.MotionReg==true && AnaDef.CSFReg==true
            matlabbatch{2}.spm.stats.fmri_spec.sess(irun).multi_reg = cellstr(sclistmot);
            if AnaDef.DerivReg
                regcounter = regcounter+0+6*2+1*2; % increase regressor-counter
            else
                regcounter = regcounter+0+6*1+1*1; % increase regressor-counter
            end
        end
        if AnaDef.GlobalMeanReg==false && AnaDef.MotionReg==true && AnaDef.CSFReg==false
            matlabbatch{2}.spm.stats.fmri_spec.sess(irun).multi_reg = cellstr(sclistmot);
            if AnaDef.DerivReg
                regcounter = regcounter+0+6*2+0*2; % increase regressor-counter
            else
                regcounter = regcounter+0+6*1+0*1; % increase regressor-counter
            end
        end
    ...
    if NumRuns==1 %% ADDED BY JR --> what to do with higher number of runs?
    ...
    elseif NumRuns > 1
        for irun=1:NumRuns
            for icond=1:AnaDef.NumCond(irun)
                tmpvec = [];
                tmpvec = [ tmpvec handles.anaobj{isubj}.Ana{1}.AnaDef.RegCond.Run{irun}.Cond{icond} ];
                handles.anaobj{isubj}.Ana{1}.AnaDef.RegCondVec.Run{irun}.vec{icond} = tmpvec;
                fprintf('Regressors: condition %d : \n',icond);
                tmpvec
            end
        end
    ...
    matlabbatch{2}.spm.stats.fmri_spec.mask             = cellstr(AnaDef.SpecMask); % explicit BASCO specification mask
    matlabbatch{2}.spm.stats.fmri_spec.mthresh          = 0; %% ADDED by JR to avoid cutting voxels
