...
% Mouse affine-reference TPM.
%
% The historical implementation used a project-specific 4-D mouse TPM.
% It is not distributed as a machine-specific absolute path. Supply it as
% job.tpm or set the environment variable NOSEMAZE_DARTEL_AFFINE_TPM.
K = 6;

if isfield(job,'tpm') && ~isempty(job.tpm)
    tpm = job.tpm;
    if iscell(tpm)
        tpm = tpm{1};
    end
else
    tpm = getenv('NOSEMAZE_DARTEL_AFFINE_TPM');
end

if isempty(tpm) || ~isfile(tpm)
    error([ ...
        'Mouse DARTEL affine-reference TPM not found. Supply job.tpm ' ...
        'or set NOSEMAZE_DARTEL_AFFINE_TPM to the historical 4-D mouse TPM.' ...
    ]);
end
