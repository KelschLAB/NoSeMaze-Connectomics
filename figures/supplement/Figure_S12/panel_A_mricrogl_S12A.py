# panel_A_mricrogl.py
#
# MRIcroGL script for Supplementary Figure 12A.
#
# Displays the rank-related deactivation map on the high-resolution
# Paxinos-space anatomical template.
#
# IMPORTANT:
...
# │
# └── results/supplement/Figure_S12/Figure_S12A/
#
...
if not os.path.isdir(input_dir):
    raise FileNotFoundError(
        'Figure 6A source-data directory not found:\n' + input_dir
    )
...
overlay_files = sorted([
    ...
    and filename.endswith('T001.nii')
])

if len(overlay_files) == 0:
    raise FileNotFoundError(
        'No file matching deactivation*T001.nii found in:\n' + input_dir
    )
...
# Remove haze / extract brain: retained exactly from original script.
gl.extract(1, 1, 5)
gl.extract(1, 1, 5)
gl.extract(1, 1, 5)
...
print('Supplementary Figure 12A rendering complete.')
