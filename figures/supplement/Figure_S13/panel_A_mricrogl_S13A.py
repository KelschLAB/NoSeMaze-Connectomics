# panel_A_mricrogl.py
#
# MRIcroGL script for Supplementary Figure 13A.
#
# Displays the vHC correlation map for deltaC on the
...
# │   └── vHC*corrdeltaC.nii
# │
# └── results/supplement/Figure_S13/Figure_S13A/
#
...
if not os.path.isdir(input_dir):
    raise FileNotFoundError(
        'Supplementary Figure 13A input directory not found:\n' + input_dir
    )

overlay_files = sorted([
    filename
    for filename in os.listdir(input_dir)
    if filename.startswith('vHC')
    and filename.endswith('corrdeltaC.nii')
])

if len(overlay_files) == 0:
    raise FileNotFoundError(
        'No file matching vHC*corrdeltaC.nii found in:\n' + input_dir
    )
...
# Slice layout retained exactly from the original script
gl.mosaic('C -31 S X R 0; A -35')
...
for filename in overlay_files:
    ...
    # Overlay settings retained exactly from the original script
    gl.minmax(1,2,5)
    gl.opacity(1,100)
    gl.colorname(1,'1BrownJR')
...
print('Supplementary Figure 13A rendering complete.')
