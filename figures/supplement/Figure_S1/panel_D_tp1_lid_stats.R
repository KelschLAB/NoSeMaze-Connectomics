...
utils::write.csv(
  myData,
  file.path(output_dir, "SourceData_Figure_S1D_TP1_stats.csv"),
  row.names = FALSE
)
...
capture.output(
  model_summary,
  file = file.path(output_dir, "Statistics_Figure_S1D_TP1_LME_summary.txt")
)
...
utils::write.csv(
  coefficient_table,
  file.path(output_dir, "Statistics_Figure_S1D_TP1_LME_coefficients.csv"),
  row.names = FALSE
)
...
utils::write.csv(
  anova_table,
  file.path(output_dir, "Statistics_Figure_S1D_TP1_LME_ANOVA.csv"),
  row.names = FALSE
)

writeLines(
  permutation_console_output,
  con = file.path(output_dir, "Statistics_Figure_S1D_TP1_PERMUTATION_RESULTS.txt")
)

saveRDS(
  permutation_result,
  file.path(output_dir, "Statistics_Figure_S1D_TP1_permutation_permlist.rds")
)
...
utils::write.csv(
  metadata,
  file.path(output_dir, "AnalysisMetadata_Figure_S1D_TP1_stats.csv"),
  row.names = FALSE
)
...
saveRDS(
  ...
  file.path(output_dir, "Figure_S1D_TP1_stats_complete_results.rds")
)

capture.output(
  sessionInfo(),
  file = file.path(output_dir, "Figure_S1D_TP1_R_sessionInfo.txt")
)

cat("\nCompleted Figure_S1D TP1 statistics.\n")
