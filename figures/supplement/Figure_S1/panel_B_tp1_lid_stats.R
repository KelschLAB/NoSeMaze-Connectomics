...
# Jonathan Reinwald
#
# Reviewer-facing statistics for Supplementary Figure_S1B.
#
# Cohort: conditioning/reappraisal
# Outcome: lid_TP1
# Model: lid_TP1 ~ block_label + (1 | animal_ID)
...
required_columns <- c("animal_ID", "block", "lid_TP1")
...
myData$lid_TP1 <- suppressWarnings(as.numeric(as.character(myData$lid_TP1)))
myData$block <- suppressWarnings(as.numeric(as.character(myData$block)))

myData <- myData[
  is.finite(myData$lid_TP1) &
...
block_label_map <- c("PRE", "TEST_81_100", "TEST_101_120", "TEST_121_140", "TEST_141_160")
names(block_label_map) <- c("1", "3", "4", "5", "6")
...
model <- lmerTest::lmer(
  lid_TP1 ~ block_label + (1 | animal_ID),
  data = myData,
  REML = TRUE
)
...
utils::write.csv(
  myData,
  file.path(output_dir, "SourceData_Figure_S1B_TP1_stats.csv"),
  row.names = FALSE
)
...
capture.output(
  model_summary,
  file = file.path(output_dir, "Statistics_Figure_S1B_TP1_LME_summary.txt")
)
...
utils::write.csv(
  coefficient_table,
  file.path(output_dir, "Statistics_Figure_S1B_TP1_LME_coefficients.csv"),
  row.names = FALSE
)
...
utils::write.csv(
  anova_table,
  file.path(output_dir, "Statistics_Figure_S1B_TP1_LME_ANOVA.csv"),
  row.names = FALSE
)

writeLines(
  permutation_console_output,
  con = file.path(output_dir, "Statistics_Figure_S1B_TP1_PERMUTATION_RESULTS.txt")
)

saveRDS(
  permutation_result,
  file.path(output_dir, "Statistics_Figure_S1B_TP1_permutation_permlist.rds")
)
...
metadata <- data.frame(
  ...
  Formula = "lid_TP1 ~ block_label + (1 | animal_ID)",
...
utils::write.csv(
  metadata,
  file.path(output_dir, "AnalysisMetadata_Figure_S1B_TP1_stats.csv"),
  row.names = FALSE
)
...
saveRDS(
  ...
  file.path(output_dir, "Figure_S1B_TP1_stats_complete_results.rds")
)

capture.output(
  sessionInfo(),
  file = file.path(output_dir, "Figure_S1B_TP1_R_sessionInfo.txt")
)

cat("\nCompleted Figure_S1B TP1 statistics.\n")
