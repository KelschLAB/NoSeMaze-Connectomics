...
# Outcome: lid
# Model: lid ~ block_label + (1 | animal_ID)
...
required_columns <- c("animal_ID", "block", "lid")
...
myData$lid <- suppressWarnings(as.numeric(as.character(myData$lid)))
...
utils::write.csv(
  myData,
  file.path(output_dir, "SourceData_Figure_S1D_tonic_stats.csv"),
  row.names = FALSE
)
...
capture.output(
  model_summary,
  file = file.path(output_dir, "Statistics_Figure_S1D_tonic_LME_summary.txt")
)
...
utils::write.csv(
  coefficient_table,
  file.path(output_dir, "Statistics_Figure_S1D_tonic_LME_coefficients.csv"),
  row.names = FALSE
)
...
utils::write.csv(
  anova_table,
  file.path(output_dir, "Statistics_Figure_S1D_tonic_LME_ANOVA.csv"),
  row.names = FALSE
)

writeLines(
  permutation_console_output,
  con = file.path(output_dir, "Statistics_Figure_S1D_tonic_PERMUTATION_RESULTS.txt")
)

saveRDS(
  permutation_result,
  file.path(output_dir, "Statistics_Figure_S1D_tonic_permutation_permlist.rds")
)
...
utils::write.csv(
  metadata,
  file.path(output_dir, "AnalysisMetadata_Figure_S1D_tonic_stats.csv"),
  row.names = FALSE
)
...
saveRDS(
  ...
  file.path(output_dir, "Figure_S1D_tonic_stats_complete_results.rds")
)

capture.output(
  sessionInfo(),
  file = file.path(output_dir, "Figure_S1D_tonic_R_sessionInfo.txt")
)

cat("\nCompleted Figure_S1D tonic statistics.\n")
