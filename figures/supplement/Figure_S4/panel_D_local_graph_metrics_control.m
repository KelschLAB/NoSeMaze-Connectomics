% panel_D_local_graph_metrics_control.m
% Jonathan Reinwald
%
...
%
% ROI labels:
%   data/processed/fMRI/Figure_3/control/roidata*.mat
%
...
%
% Output:
%   results/supplement/Figure_S4/Figure_S4D/
%
% -------------------------------------------------------------------------
...
preferred = find(contains({roiFiles.name}, 'v6'), 1, 'first');
...
fig = figure( ...
    'Name', 'Figure S4D: regional local graph metrics', ...
    'Visible', 'on', ...
    'Color', 'white', ...
...
sgtitle( ...
    'Figure S4D | control cohort | TPnoPuff TEST > PRE', ...
    'Interpreter', 'none' ...
);
...
for metricIndex = 1:numel(localMetrics)
    ...
    writetable( ...
        ...
        fullfile( ...
            outputDir, ...
            sprintf('SourceData_Figure_S4D_%s.csv', metricName) ...
        ) ...
    );
    ...
    writetable( ...
        preTable, ...
        fullfile(outputDir, sprintf('SourceData_Figure_S4D_%s_PRE.csv', metricName)) ...
    );

    writetable( ...
        testTable, ...
        fullfile(outputDir, sprintf('SourceData_Figure_S4D_%s_TEST.csv', metricName)) ...
    );
    ...
    writetable( ...
        ...
        fullfile( ...
            outputDir, ...
            sprintf('AnalysisMetadata_Figure_S4D_%s.csv', metricName) ...
        ) ...
    );
end
...
save( ...
    fullfile(outputDir, 'Statistics_Figure_S4D_complete_results.mat'), ...
...
exportgraphics( ...
    fig, ...
    fullfile(outputDir, 'Figure_S4D_local_graph_metrics.pdf'), ...
    'ContentType', 'vector', ...
...
exportgraphics( ...
    fig, ...
    fullfile(outputDir, 'Figure_S4D_local_graph_metrics.png'), ...
    'Resolution', 300, ...
    'BackgroundColor', 'white' ...
);
...
savefig( ...
    fig, ...
    fullfile(outputDir, 'Figure_S4D_local_graph_metrics.fig') ...
);

fprintf('\nCompleted Supplementary Figure S4D.\n');
