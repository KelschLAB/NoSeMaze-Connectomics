% panel_F_deltaL_BOLD.m
% Jonathan Reinwald
%
% Figure 6F: whole-brain Delta L adaptation vs BOLD extracted from the
% Delta-L-sensitive vHC cluster shown in Figure 6E.
%
% Delta L predictor:
...
%
% Required beta file:
%   data/processed/fMRI/Figure_6/Figure_6F/mask_vHC_T01_corrdeltaL.mat
%
% The beta MAT file must contain:
...
aucDir = fullfile( ...
    repoRoot,'data','processed','fMRI','Figure_4','Figure_4C_E', ...
    'conditioning' ...
);

inputDir = fullfile( ...
    repoRoot,'data','processed','fMRI','Figure_6','Figure_6F' ...
);

outputDir = fullfile( ...
    repoRoot,'results','main','Figure_6','Figure_6F' ...
);
...
aucPreFile = fullfile( ...
    aucDir,'auc_struc_TPnoPuff11to40_45to50_p.mat' ...
);

aucTestFile = fullfile( ...
    aucDir,'auc_struc_TPnoPuff81to120_45to50_p.mat' ...
);

betaFile = fullfile( ...
    inputDir,'mask_vHC_T01_corrdeltaL.mat' ...
);
...
if ~isfield(aucPre,'g_delta_L') || ~isfield(aucTest,'g_delta_L')
    error('g_delta_L missing from AUC structure.');
end

deltaLPre = [aucPre.g_delta_L]';
deltaLTest = [aucTest.g_delta_L]';
deltaLInput = deltaLTest - deltaLPre;

%% Load extracted BOLD values

[betaPre,betaTest,betaDiff] = extractBetaVectors(betaFile);

if numel(deltaLInput) ~= numel(betaPre)
    error( ...
        'Sample-size mismatch: DeltaL n=%d; beta n=%d.', ...
        numel(deltaLInput), ...
        numel(betaPre) ...
    );
end

fprintf('Figure 6F: n=%d\n',numel(deltaLInput));
...
writetable( ...
    sourceData, ...
    fullfile(outputDir,'SourceData_Figure_6F.csv') ...
);

fprintf('Figure 6F completed.\n');
...
function makeFigure( ...
    ...
    writetable( ...
        statsTable, ...
        fullfile(outputDir,['Statistics_' panelName '.csv']) ...
    );
    ...
    save( ...
        fullfile(outputDir,['Results_' panelName '.mat']), ...
        'result' ...
    );
    ...
    exportgraphics( ...
        fig, ...
        fullfile(outputDir,[panelName '.pdf']), ...
        'ContentType','vector', ...
    ...
    exportgraphics( ...
        fig, ...
        fullfile(outputDir,[panelName '.png']), ...
        'Resolution',300, ...
        'BackgroundColor','white' ...
    );
end
